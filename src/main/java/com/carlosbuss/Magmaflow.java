...
// Add this method to your VolcanoPlotJavaFX class (around line 200, after the Gene class definition)

/**
 * Predefined canonical target gene categories for quick selection
 */
...
/**
 * Show dialog for selecting canonical target gene categories
 */
private void showCanonicalTargetsDialog(Stage stage) {
    ...
    private double xLabelDistance = 40;  // Distance of X-axis label from the axis
    private double yLabelDistance = 40;  // Distance of Y-axis label from the axis
...
public void start(Stage primaryStage) {
    ...
    Button canonicalTargetsButton = new Button("Add Canonical Targets");
    ...
    CheckBox hoverInfoCheck = new CheckBox("Show gene info on hover");
    hoverInfoCheck.setSelected(showHoverInfo);

    // Add this after the zeroLineCheck
CheckBox transparentDotsCheck = new CheckBox("Transparent dots");
transparentDotsCheck.setSelected(false);

// Add a slider for transparency control
Label opacityLabel = new Label("Dot Opacity: 100%");
...
private double parseDoubleSafe(String value) {
    ...
    try {
        ...
        return Double.parseDouble(value.replaceAll("[^0-9Ee.-]", ""));
...
private void drawPlot() {
    ...
    double xLabelBaseY = canvasHeight - marginBottom + xLabelDistance; // was: + 40
    xLabelNode.setX(xLabelBaseX);
    xLabelNode.setY(xLabelBaseY);
    plotPane.getChildren().add(xLabelNode);

    // Y axis label (rotated, centered) - REPLACE THIS SECTION
    ...
    double baseX = marginLeft - yLabelDistance;  // CHANGED: use yLabelDistance
    ...
    // Compute dot positions ahead of drawing
    for (Gene g : genes) {
        double pValToUse = useAdjustedP ? g.pAdj : g.pValue;
    ...
    // Draw all gene dots with outline support
    for (Gene g : genes) {
        double pValToUse = useAdjustedP ? g.pAdj : g.pValue;
        ...
        if (showHoverInfo) {
            ...
            // Remove any existing tooltip and clear event handlers
            javafx.scene.control.Tooltip.uninstall(circle, null);
            circle.setOnMouseEntered(null);
            circle.setOnMouseExited(null);
        }

        plotPane.getChildren().add(circle); // THIS WAS MISSING - ADD THIS LINE!
    } // THIS CLOSING BRACE WAS MISSING - the for-loop that draws dots needs to end here
    ...
    for (Gene g : genes) {
        ...
        double endX = (g.logFC < 0) ? g.labelX + textWidth : g.labelX;
...
private void saveProject(Stage stage) {
    ...
    try (PrintWriter out = new PrintWriter(file)) {
        ...
        // FIX: Add the missing xLabel and yLabel saves
        out.printf("    \"xLabel\": \"%s\",%n", xLabel.replace("\"", "\\\""));
        out.printf("    \"yLabel\": \"%s\",%n", yLabel.replace("\"", "\\\""));
        out.printf("    \"xLabelDistance\": %.1f,%n", xLabelDistance);
        out.printf("    \"yLabelDistance\": %.1f,%n", yLabelDistance);
        out.printf("    \"dotOpacity\": %.2f,%n", dotOpacity);
        out.printf("    \"showDotOutline\": %b,%n", showDotOutline);
        out.printf("    \"dotOutlineColor\": \"%s\",%n", toHex(dotOutlineColor));
        out.printf("    \"dotOutlineWidth\": %.2f,%n", dotOutlineWidth);
        out.printf("    \"showHoverInfo\": %b,%n", showHoverInfo);
        // Remove the trailing comma from the last line by adding the closing brace
        out.println("  }");
        out.println("}");
...
private void loadProject(Stage stage) {
    ...
    try (BufferedReader br = new BufferedReader(new FileReader(file))) {
        ...
        for (String entry : genesBlock.split("\\},\\s*\\{")) {
            ...
            // Extract label positions if they exist
        ...
        if (json.contains("\"settings\"")) {
            ...
            // Set defaults if not found in saved project
           ...
           // Hover info setting
            if (json.contains("\"showHoverInfo\"")) {
                ...
                showHoverInfo = true; // Default to enabled if not found
            } 
