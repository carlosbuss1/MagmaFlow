...
private void updateAnnotations() {
    // Handle target genes visibility
    for (Gene gene : genes) {
        boolean isTargetGene = targetedGenes.contains(gene.name);
        boolean shouldShowTarget = showTargets && isTargetGene;
        
        if (gene.labelNode != null && gene.lineNode != null) {
            // Only modify visibility if this is a target gene
            if (isTargetGene) {
                gene.labelNode.setVisible(shouldShowTarget);
                gene.lineNode.setVisible(shouldShowTarget);
            }
        }
    }
    
    // Redraw to see changes immediately
    drawPlot();
}
}
