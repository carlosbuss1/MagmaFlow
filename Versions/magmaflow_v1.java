...
public class VolcanoPlotJavaFX extends Application {
    ...
    class Gene {
        ...
        double pValue;
        double pAdj;
        // Plot coordinates for the dot
        double dotX;
        double dotY;
        // Label location (persisted across redraws)
        double labelX;
        double labelY;
        // Flag indicating whether the user has manually positioned this label
    ...
    public void start(Stage primaryStage) {
        ...
        // to a BooleanProperty in the targetGeneSelections map.  We
        // allocate a fixed height so it fits within the control panel.
        ...
        targetedGeneListView.setCellFactory(CheckBoxListCell.forListView(item -> {
            return targetGeneSelections.computeIfAbsent(item, k -> new SimpleBooleanProperty(true));
        }));
        Label targetListLabel = new Label("Toggle Target Genes:");
        targetListLabel.setFont(Font.font("Arial", fontSize));
        // Append the target gene toggle list to the controls.  Insert it near
        // the end but before the geneInfo label so that it appears just
        // above the gene info display.
    ...
    result.ifPresent(genesText -> {
        String[] genesArray = genesText.split("[,\\s]+"); // ✅ Correctly escaped
        targetedGenes.clear();
    ...
    private void loadTargetGenes(Stage stage) {
        ...
        // Accept both plain text and CSV files for gene lists
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(file))) {
            ...
            while ((line = br.readLine()) != null) {
                // Split each line on commas or whitespace to support CSV or plain text lists
...
private void updateAnnotations() {
    for (Gene gene : genes) {
        boolean annotate = targetedGenes.contains(gene.name);
        if (gene.labelNode != null && gene.lineNode != null) {
            gene.labelNode.setVisible(annotate);
            gene.lineNode.setVisible(annotate);
        }
    }
}
}
